...
When the user clicks the button, the extension checks the current tab and its frames for a compatible URL exposed by the page resource list, PDF viewer element, or PDF.js viewer state. This means it does not require reloading the portal page. Chrome then requests that URL through the normal browser session and shows its native save dialog.
